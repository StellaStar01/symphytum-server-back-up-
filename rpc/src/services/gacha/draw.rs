...
// One drawn card -> its response entry. card_reward_results order must match the
// real server: card, then costume (if any), then sd costume (if any), then
// acquire_reward (if any). duplicate_reward_type = NONE (no state => always new).
...
// Rolls `count` cards. The guaranteed slot applies ONLY when `guarantee` is
// true (button.fixed_reward_pick_count > 0) AND the batch rolled no R4+ card:
// the LAST result is replaced with a card picked uniformly from
// `guaranteed_pool` when Some(non-empty), else with an R4+ roll from `pool`
// (weights normalized over the R4/R5 tiers).
pub fn draw(
    rng: &mut impl Rng,
    pool: &[&Card],
    count: usize,
...
mod tests {
    ...
    async fn per_card_uniform_hits_all_rarities() {
        ...
        let ids: std::collections::HashSet<&str> =
            results.iter().map(|r| r.card_id.as_str()).collect();
        let all_ids: std::collections::HashSet<&str> = pool.iter().map(|c| c.id.as_str()).collect();
        assert!(
